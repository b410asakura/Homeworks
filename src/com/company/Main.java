...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < students1.length; i++) {
            randomBithdays[i] = random.nextInt(31) + "." + random.nextInt(13) + "." + (random.nextInt(2005 - 1995) + 1995);
        }
        for (int i = 0; i < students1.length; i++) {
        ...
        course1.printInfo();
        for (int i = 0; i < students1.length; i++) {
            students1[i] = new Student(randomBithdays[i], randomSurnames[i], randomNames[i], randomSex[i], random.nextInt(24));
